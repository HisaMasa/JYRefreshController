...
{
  if (!(self.canRefreshDirection & direction) || (self.refreshingDirection & direction)) {
    return;
  }
  ...
  switch (direction) {
    ...
    case kJYRefreshDirectionBottom:
      ...
      contentOffset = CGPointMake(0, _scrollView.contentSize.height - _scrollView.bounds.size.height + contentInset.bottom);
      break;
    default:
      break;
  }
...
- (void)_checkOffsetsForDirection:(JYRefreshDirection)direction change:(NSDictionary *)change {
  ...
  refreshView.visible = didShowHeight >= 0;
